...
sub = pd.read_csv('./Submit.csv',encoding='gbk')
sub['销售日期_D'] = sub['日期'].map(lambda x:str(x)[-2:])
sub['销售日期_D'] = sub['销售日期_D'] .astype(int)
result = sub.copy()
# print(sub)

sub_train = sub[['编码','销售日期_D','销量','日期']]
sub_train['日期'] = sub_train['日期'].map(lambda x: str(x)[:4] + '-' + str(x)[4:6] + '-' + str(x)[6:])
sub_train['日期_W_5'] = pd.to_datetime(sub_train['日期']).map(lambda x :x.weekday()+1)
del sub_train['日期']
...
train.drop(['大类名称','中类名称','小类名称','销售月份','商品编码','小类编码','custid'],axis=1,inplace=True)
print('月份总数',list(train['销售日期_M'].unique()))

train['销售日期'] = train['销售日期'].map(lambda x: str(x)[:4] + '-' + str(x)[4:6] + '-' + str(x)[6:])
train['销售日期_W'] = pd.to_datetime(train['销售日期']).map(lambda x :x.weekday()+1)
train_weekday = train[['销售日期_W','销售日期_D','销售日期']].drop_duplicates(['销售日期_W','销售日期_D','销售日期'])

train_weekday_1 = train_weekday[train_weekday['销售日期']<'2015-02-01']
train_weekday_1.rename(columns={'销售日期_W':'销售日期_W_1'},inplace=True)
train_weekday_2 = train_weekday[(train_weekday['销售日期']<'2015-03-01')&(train_weekday['销售日期']>='2015-02-01')]
train_weekday_2.rename(columns={'销售日期_W':'销售日期_W_2'},inplace=True)
train_weekday_3 = train_weekday[(train_weekday['销售日期']<'2015-04-01')&(train_weekday['销售日期']>='2015-03-01')]
train_weekday_3.rename(columns={'销售日期_W':'销售日期_W_3'},inplace=True)
train_weekday_4 = train_weekday[(train_weekday['销售日期']<'2015-05-01')&(train_weekday['销售日期']>='2015-04-01')]
train_weekday_4.rename(columns={'销售日期_W':'销售日期_W_4'},inplace=True)
...
train = pd.merge(train,train_weekday_1[['销售日期_D','销售日期_W_1']],on=['销售日期_D'],how='left')
train = pd.merge(train,train_weekday_2[['销售日期_D','销售日期_W_2']],on=['销售日期_D'],how='left')
train = pd.merge(train,train_weekday_3[['销售日期_D','销售日期_W_3']],on=['销售日期_D'],how='left')
train = pd.merge(train,train_weekday_4[['销售日期_D','销售日期_W_4']],on=['销售日期_D'],how='left')
...
train_lgb = train[['编码','销售日期_D','is_buy_2','is_buy_3','是否促销_2','销售日期_W_3']]
print(train_lgb)

# train_lgb['base_1'] = train_lgb['is_buy_2'] / (1 / (train_lgb['销售日期_D']))
del train_lgb['销售日期_D']
...
test_lgb = train[['编码','销售日期_D','is_buy_3','is_buy_4','是否促销_3','销售日期_W_4']]
...
sub_lgb = train[['编码','销售日期_D','is_buy_4','是否促销_4','日期_W_5']]
